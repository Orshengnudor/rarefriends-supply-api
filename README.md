...
`RPC_URL_PRIMARY` is your keyed RPC (Alchemy). `RPC_URL_FALLBACK` is
`https://rpc.mainnet.chain.robinhood.com` and is used automatically when the primary fails.
The key stays server-side; it is never exposed to the browser.
